...
# The fixed test case (file1001429, slice 020) as a typed dict, for
# --conditioning_mode schema. clip_plain/clip_xml/unimed_clip instead read
# the string from ./assets/fastmri/knee/file1001429/meta/prompt.txt, same as
# before this change. These field values match the same scan as it appears
# in the knee-testplan SCANS list (file1001429 / slice 20 / PDFS / healthy /
# TurboSpinEcho / TR 3120 / TE 33 / TI 100 / FA 140) - verify against your
# own metadata CSV if you have one; this wasn't independently re-derived
# from raw fastMRI headers.
FIXED_TEST_CASE_METADATA = {
    "anatomy": "knee", "slice": 20.0, "contrast": "PDFS", "pathologies": [],
    "sequence": "Turbospinecho", "TR": 3120.0, "TE": 33.0, "TI": 100.0, "flip_angle": 140.0,
}
# end
...
def main(args):
    ...
    #DW
    if args.conditioning_mode == "schema":
        prompt = dict(FIXED_TEST_CASE_METADATA)  # see module-level comment
    else:
    ...
    if args.cfg_scale == 0:
        # DW unconditional: an empty dict for schema, an empty string for the three string-based arms
        prompt = [{} if args.conditioning_mode == "schema" else ""] * B
...
if __name__ == '__main__':
    ...
    parser = add_conditioning_args(parser) # DW

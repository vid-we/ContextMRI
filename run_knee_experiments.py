...
def generate_html_report(results: list, output_path: str):
    ...
    for r in sorted(results, key=lambda x: -x["psnr"]):
        ...
        <tr>
          ...
          <td>{r['cfg_scale']}</td>
          <td class="meta">{r['meta_name']}</td>
          <td class="{'good' if r['psnr'] > 30 else 'bad'}">{r['psnr']:.2f}</td>
          <td>{r['ssim']:.4f}</td>
          <td>{r['lpips']:.4f}</td>
        </tr>"""
...
<head>
<meta charset="UTF-8">
<title>ContextMRI Knie-Testplan</title>
<style>
  body {{ font-family: 'Courier New', monospace; background: #0f0f0f; color: #e0e0e0; padding: 2rem; }}
  h1   {{ color: #7ecfff; letter-spacing: 2px; }}
  h2   {{ color: #aaa; font-size: 0.9rem; margin-top: 2rem; }}
  table {{ border-collapse: collapse; width: 100%; margin-top: 1rem; font-size: 0.85rem; }}
  th   {{ background: #1e1e2e; color: #7ecfff; padding: 8px 12px; text-align: left; border-bottom: 1px solid #333; }}
  td   {{ padding: 6px 12px; border-bottom: 1px solid #1e1e1e; }}
  tr:hover td {{ background: #1a1a2a; }}
  .good {{ color: #7fff7e; font-weight: bold; }}
  .bad  {{ color: #ff7e7e; }}
  .meta {{ color: #ffcc77; font-size: 0.8rem; }}
  .generated {{ color: #666; font-size: 0.75rem; margin-top: 2rem; }}
</style>
</head>
<body>
<h1>ContextMRI — Knie Testplan Report</h1>
<p>Generiert: {datetime.now().strftime('%Y-%m-%d %H:%M')}</p>
<h2>ALLE EXPERIMENTE (sortiert nach PSNR (down))</h2>
<table>
  <thead>
    <tr>
      <th>#</th><th>Maske</th><th>Acc</th><th>CFG</th>
      <th>Metadaten</th><th>PSNR (up)</th><th>SSIM (up)</th><th>LPIPS (down)</th>
    </tr>
  </thead>
  <tbody>{rows}</tbody>
</table>
<p class="generated">Datenquelle: {RESULTS_JSON}</p>
</body>
</html>"""
